...
function rsvToBlock(rsv: Reservation, minTime: number, inc: number, resourceNames: string[]): Block {
    ...
    if (rsv.category === 'pool') {
        startSpaces =
            rsv.lanes?.[0] !== 'auto' ? rsv.lanes?.map((lane) => occ * resourceNames.indexOf(lane)) ?? null : null;
    } else if (rsv.category === 'classroom') {
...
function insertUnAssigned(
    ...
    const bestPath = getMinBreaksPath(spacesByTimes, laneWidth, blk.width, blk.startTime, blk.endTime);
    if (bestPath) {
...
export function oldAssignPoolSpaces(rsvs: Reservation[], dateStr: string): {
    status: string;
...
const getStyleType = (width: number, startSpace: number | null, space: number): 'single' | 'start' | 'middle' | 'end' => {
    if (width <= 1) {
        return 'single';
    } else {
