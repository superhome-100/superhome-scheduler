import type { OWReservation, OWSubmission, Submission } from '$types';
import { ReservationType } from '$types';

const sortBuddies = (grp: OWReservation[]) => {
	//deepest to shallowest
	return grp.sort((a, b) => b.maxDepth - a.maxDepth);
};

function sortBuddyGroups(buddyGrps: OWReservation[][]) {
	//deepest to shallowest
...
function createBuddyGroups(rsvs: OWReservation[]) {
	let remaining = [...rsvs];
	let grps = [];
	let isBuddy = (a: Submission, b: Submission) => a.buddies.includes(b.user!) && a.buoy == b.buoy;
...
const largeDepthDifference = (
	deepBg: OWSubmission[],
	shallowBg: OWSubmission[],
	threshold: number
) => {
...
function createBuoyGroupsFromBuddyGroups(buddyGrps: OWReservation[][], maxDepthDiff: number) {
	...
	const createBuoyGroup = (bgs: OWReservation[][], idxs: number[]) => {
	...
	const matchOne = (curBg: OWReservation[], searchIdx: number, diffThresh: number) => {
	...
	const forceAddSoloDiver = (solo: OWReservation) => {
		let added = false;
...
export function createBuoyGroups(rsvs: OWReservation[], maxDepthDiff: number) {
