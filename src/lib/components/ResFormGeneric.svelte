<script lang="ts">
	import type { BuddyData, Reservation, UserMinimal } from '$types';
	...
	const initBF = (): BuddyData[] => {
		...
		if (rsv != null) {
			for (let i = 0; i < rsv.buddies.length; i++) {
				const buddy = $storedUsers
					? $storedUsers[rsv.buddies[i]] ?? fallbackBuddy(rsv.buddies[i])
					: fallbackBuddy(rsv.buddies[i]);
				buddyFields.push({
					name: buddy.nickname,
