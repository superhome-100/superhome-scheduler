<script lang="ts">
	...
	const showAdminCommentForm = (date: string, buoy: string, owTime: OWTimeT) => {
		open(AdminComment, { date, buoy, owTime });
	};
	...
	const getBuoyGroupings = (
		owTime: OWTimeT,
		reservations: ReservationEx[],
		boatAssignments: typeof $storedBoatAssignments,
		buoys: typeof $storedBuoys,
		comments: typeof $storedOWAdminComments
	) => {
		if (!reservations.length || !buoys.length) return [];
		const reservationswithBuoy = setBuoyToReservations(
			buoys,
			reservations.filter((r) => r.owTime === owTime) as OWReservation[]
		);
		return buoys
			.map((v) => {
				const comment = comments.find((c) => c.buoy === v.name && c.am_pm === owTime);
				const buoyReservation = reservationswithBuoy.filter((r) => r._buoy === v.name);
				return {
					id: `group_${v.name}`,
					buoy: v,
					boat: boatAssignments?.[v.name!] || null,
					reservations: buoyReservation,
					adminComment: comment?.comment ?? '',
					headCount: getHeadCount(buoyReservation)
				};
			})
	...
	$: sessionsObj = {
		[OWTime.AM]: {
			owTime: OWTime.AM,
			isOpen: $storedSettings.getOpenwaterAmBookable(date),
			isFull: isAmFull,
			buoyGroupings: getBuoyGroupings(
				'AM',
				reservations,
				$storedBoatAssignments,
				$storedBuoys,
		...
		[OWTime.PM]: {
			owTime: OWTime.PM,
			isOpen: $storedSettings.getOpenwaterPmBookable(date),
			isFull: false,
			buoyGroupings: getBuoyGroupings(
				'PM',
				reservations,
	...
	$: sessions = [sessionsObj[OWTime.AM], sessionsObj[OWTime.PM]];
...
{/if}
{#if isOpen || isAdmin}
	{#each sessions as session (session.owTime)}
		<section class="w-full relative block">
			...
			>
				...
				<div class="grow text-center justify-items-center">
					<span>{session.owTime}</span><span class="desktop-text">&nbsp;Session</span>
					{#if !session.isOpen}
						...
						>
							...
							{#each boats as boat (boat)}
								...
								<span class="bg-teal-100 border border-black px-0.4"
									>{session.buoyGroupings
										.filter((b) => b.boat === boat)
										.reduce((a, b) => a + b.headCount, 0)}</span
								>
							{/each}
						</div>
					{/if}
			...
			<ul class="flex flex-col gap-0.5 sm:gap-3">
				{#each session.buoyGroupings as grouping (grouping.id)}
					<li
					...
					>
						<div class="flex-none w-12 min-w-12">
							<button
								...
								on:click={() =>
									isAdmin && showAdminCommentForm(date, grouping.buoy.name, session.owTime)}
							>
								...
								<select
									...
									on:input={async (e) => {
										toast
											...
											.catch((e) => console.warn('boat-assignment', grouping.buoy.name, e));
									}}
								>
						...
						<div class="grow flex w-auto relative gap-0.5 sm:gap-2">
							<div class="w-full">
								<DayOpenWaterSubmissionsCard
									buoy={grouping.buoy}
									submissions={grouping.reservations}
									onClick={() => {
										showViewRsvs(grouping.reservations);
									}}
									adminComment={grouping.adminComment}
									adminView={isAdmin}
								/>
							</div>
						</div>
