...
const minStart = (stns, dateStr, cat) => cat === 'pool' ? minPoolStart(stns, dateStr) : cat === 'classroom' ? minClassroomStart(stns, dateStr) : undefined;
const maxEnd = (stns, dateStr, cat) => cat === 'pool' ? maxPoolEnd(stns, dateStr) : cat === 'classroom' ? maxClassroomEnd(stns, dateStr) : undefined;

const nRes = (stns, dateStr, cat) => Math.floor((maxEnd(stns, dateStr, cat) - minStart(stns, dateStr, cat)) / inc(stns, dateStr))

export const startTimes = (stns, dateStr, cat) => Array(nRes(stns, dateStr, cat))
    .fill()
    .map((v,i) => dtu.minToTimeStr(minStart(stns, dateStr, cat) + i*inc(stns, dateStr)));

export const endTimes = (stns, dateStr, cat) => Array(nRes(stns, dateStr, cat))
    .fill()
