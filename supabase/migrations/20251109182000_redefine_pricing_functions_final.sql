...
-- Final authoritative version of pricing functions
-- Picks the most recent price_template_updates.created_at at or before the reservation moment;
-- if none exist before that, falls back to the latest available update for the user's template.
...
COMMENT ON FUNCTION public.compute_prices_for_reservation(uuid, date)
  IS 'Prices effective as of reservation date; if none before date, use latest available update for the template.';
...
COMMENT ON FUNCTION public.compute_prices_for_reservation_at(uuid, timestamptz)
  IS 'Prices effective as of reservation timestamp; if none before timestamp, use latest available update for the template.';
