-- Update auto_assign_buoy function to use correct course_coaching variable instead of display name
set search_path = public;

-- Update the auto_assign_buoy function to use the correct open_water_type variable
...
begin
  ...
  loop
    -- Special-case: course_coaching should never be grouped with others
    if v_reservation.open_water_type = 'course_coaching' then
...
comment on function public.auto_assign_buoy(date, varchar) is 'Auto-assigns buoys to groups, excluding course_coaching from grouping (creates single-member groups for course_coaching)';
