<script lang="ts">
  ...
  const getColumnConfig = (type: string) => {
    switch (type) {
      case 'buoy':
        return {
          class: 'resizable-header w-36 min-w-28',
      ...
      case 'boat':
        return {
          class: 'resizable-header w-40 min-w-32',
      ...
      case 'divers':
        return {
          class: 'resizable-header w-auto min-w-48',
      ...
      default:
        return {
          class: 'resizable-header w-auto min-w-32',
          icon: '',
          label: 'COLUMN'
        };
    }
...
<th class={config.class} data-column={columnType} on:mousedown={(e) => onMouseDown(e, columnType)}>
