<script lang="ts">
  ...
  const getDisplayName = (res: any): string => {
    ...
    if (isAdmin) {
      ...
      let name = res?.user_profiles?.name || res?.user_profiles?.nickname || res?.user_profiles?.username || (Array.isArray(res?.member_names) && res.member_names[0]) || res?.nickname || res?.username || res?.name || res?.title || '';
