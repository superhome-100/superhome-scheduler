<script lang="ts">
  ...
  const handleReservationClick = (reservation: any) => {
    console.log("ClassroomCalendar: Reservation clicked", {
      isAdmin,
      reservation,
    });
  ...
  const getDisplayName = (res: any): string => {
    ...
    if (isAdmin) {
      ...
      let name =
        ...
        (Array.isArray(res?.member_names) && res.member_names[0]) ||
        res?.nickname ||
        res?.username ||
        res?.name ||
        res?.title ||
...
>
  ...
  <div class="relative max-h-[50vh] sm:max-h-[60vh] overflow-y-auto">
    ...
    <div
      class="grid"
      style={`grid-template-columns: 80px repeat(${ROOMS.length}, 1fr); grid-template-rows: ${rowTemplate};`}
    >
      <!-- Calendar Header -->
      <div
    ...
    <div
      ...
      style={`grid-template-columns: 80px repeat(${ROOMS.length}, 1fr); grid-template-rows: ${rowTemplate}; z-index: 10;`}
    >
      {#if displayReservations && displayReservations.length}
        {#each ROOMS as _roomLabel, ridx}
          ...
          <div
            class="relative"
            style={`grid-column: ${ridx + 2} / ${ridx + 3}; grid-row: 2 / ${2 + hourSlots.length};`}
          >
            {#each displayReservations
              ...
              .filter((x) => !!x.rect)
              .sort((a, b) => a.rect!.topPx - b.rect!.topPx) as item (`${resKey(item.r)}-${item.rIdx}`)}
              {@const reservation = item.r}
              {@const rect = item.rect}
              {#if rect}
                <div
                  class="pointer-events-auto absolute left-1 right-1 rounded-lg text-[0.7rem] sm:text-sm cursor-pointer hover:font-semibold bg-gradient-to-br from-red-100 to-red-200 text-red-900 border border-red-300 flex flex-col justify-center p-2 overflow-hidden z-10"
