<script lang="ts">
  ...
  async function bulkUpdatePendingReservations(
    ...
    try {
      ...
      const reservations = (pendingReservations || []).map((r) => ({
        uid: r.uid,
        res_date: r.res_date,
      }));
  ...
  const handleCalendarDateClick = (event: CustomEvent) => {
    ...
    initialSingleDayType =
      typeof detail === "string"
        ? initialSingleDayType
        : detail?.type || initialSingleDayType;
  ...
  const loadAdminData = async () => {
    try {
      ...
      console.log(
        "AdminDashboard: Successfully loaded users:",
        usersData?.length || 0,
      );
      ...
      try {
        ...
        if (!reservationsError) {
          ...
          pendingReservationsData = reservationsData.filter(
            (r) => r.res_status === "pending",
          );
          console.log(
            "AdminDashboard: Successfully loaded and transformed reservations:",
            reservationsData.length,
          );
        }
      } catch (resErr) {
        console.log(
          "AdminDashboard: Reservations table not found, skipping...",
        );
      }
  ...
  const toggleUserPrivilege = async (
    uid: string,
    ...
    try {
      ...
      if (!res.success)
        throw new Error(res.error || "Failed to update user privilege");
      await loadAdminData();
    } catch (err) {
      console.error("Error toggling user privilege:", err);
      error =
        err instanceof Error ? err.message : "Failed to update user privilege";
    }
  };
  ...
  const updateUserNickname = async (uid: string, nickname: string) => {
    try {
      ...
      if (!res.success)
        throw new Error(res.error || "Failed to update nickname");
      await loadAdminData();
    } catch (err) {
  ...
  const updateUserSubscriberType = async (
    uid: string,
    price_template_name: string,
  ) => {
    try {
      const res = await userAdminService.updatePriceTemplate(
        uid,
        price_template_name,
      );
      if (!res.success)
        throw new Error(res.error || "Failed to update subscriber type");
      await loadAdminData();
    } catch (err) {
      console.error("Error updating subscriber type:", err);
      error =
        err instanceof Error ? err.message : "Failed to update subscriber type";
    }
  };
  ...
  $: if (loading) {
    ...
    loadingTimeout = setTimeout(() => {
      console.warn(
        "AdminDashboard: Loading timeout - forcing loading to false",
      );
      loading = false;
  ...
  <PullToRefresh onRefresh={handleRefresh} {refreshing}>
    <BodyContent>
      {#if loading}
        ...
        >
          <LoadingSpinner />
          <p class="mt-4 text-base-content/70 text-base">
            Loading admin data...
          </p>
        </div>
      {:else if error}
        <div
          class="bg-base-100 rounded-xl p-8 text-center shadow-sm border border-error/20"
        >
          <h2 class="text-error text-xl mb-4 font-semibold">
            Error Loading Data
          </h2>
      ...
      {:else}
        ...
        {#if adminView === "dashboard"}
          <!-- Single active button with dropdown for section switching - sticky on mobile -->
          <div
            class="sticky top-0 z-20 bg-base-200/90 backdrop-blur-md py-4 transition-all duration-200 sm:relative sm:bg-transparent sm:backdrop-blur-none sm:py-0 sm:mb-8"
          >
            <AdminSectionSwitcher
              selected={adminSection}
              on:select={(e) => (adminSection = e.detail.section)}
            />
          </div>
          ...
          {#if adminSection === "pending"}
            <PendingReservations
              ...
              on:reservationAction={(e: any) =>
                handleReservationAction(e.detail.reservation, e.detail.action)}
              on:openReservationDetails={(e: any) =>
                openReservationDetails(e.detail)}
            />
          {:else if adminSection === "user_res"}
            <AdminUserReservations
              {reservations}
              on:openReservationDetails={(e) =>
                openReservationDetails(e.detail)}
            />
        ...
        {:else if adminView === "users"}
          <UserManagement
            ...
            {priceTemplates}
            on:refresh={handleRefresh}
            on:toggleUserStatus={(e: any) =>
              toggleUserStatus(e.detail.uid, e.detail.currentStatus)}
            on:toggleUserPrivilege={(e: any) =>
              toggleUserPrivilege(e.detail.uid, e.detail.currentPrivileges)}
            on:updateNickname={(e: any) =>
              updateUserNickname(e.detail.uid, e.detail.nickname)}
            on:updateSubscriberType={(e: any) =>
              updateUserSubscriberType(
                e.detail.uid,
