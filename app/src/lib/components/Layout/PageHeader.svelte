...
<div class="page-header">
  <div class="px-3 py-3 sm:px-4 sm:py-4 md:px-6 md:py-5 lg:px-8 lg:py-6 xl:px-10 xl:py-6 2xl:px-12 2xl:py-6 max-w-7xl mx-auto w-full">
    <div class="flex items-center gap-4 header-content">
      ...
      <div class="flex-1">
        <h1 class="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold mb-1 sm:mb-2" style="color: #00294C;">{title}</h1>
        {#if subtitle}
          <p class="text-sm sm:text-base" style="color: #00294C;">{subtitle}</p>
        {/if}
      </div>
    </div>
  </div>
